 ...
 *  の責任を負わない．
 *
 *  $Id: sample1_vm4.c 739 2017-01-24 10:05:05Z nces-hibino $
 */
...
vmcomtest(void)
{
	...
	*((unsigned int *)writedata) = vmcontest_cnt;
	ercd = WriteStateVariable(SVAR4, writedata);
	if (ercd != E_OK) {
		syslog(LOG_INFO, "WriteStateVariable(SVAR4) error! ercd = %d", ercd);
	} else {
		syslog(LOG_INFO, "WriteStateVariable(SVAR4) = %d", *((unsigned int *)writedata));
	}
	...
	ercd = WriteMessageQueue(MSGQ4, writedata, 4);
	if (ercd != E_OK) {
		syslog(LOG_INFO, "WriteMessageQueue(MSGQ4) error! ercd = %d", ercd);
	}
	else {
		syslog(LOG_INFO, "WriteMessageQueue(MSGQ4) = %d", *((unsigned int *)writedata));
	}

	ercd = ReadMessageQueue(MSGQ3, readdata);
	if (ercd < 0) {
		syslog(LOG_INFO, "ReadMessageQueue(MSGQ3) error! ercd = %d", ercd);
	}
	else {
		syslog(LOG_INFO, "ReadMessageQueue(MSGQ3) = %d", *((unsigned int *)readdata));
	}
...
GetCommand(void)
{
	...
	do {
		...
		cnt++;
		if (cnt == 100) {
			syslog(LOG_INFO, "ATK2 on VM4 : running! (%d)", run_cnt++);
			cnt = 0;
			vmcomtest();
		}
	} while (command == '\0');
