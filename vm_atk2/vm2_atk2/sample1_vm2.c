 ...
 *  の責任を負わない．
 *
 *  $Id: sample1_vm2.c 739 2017-01-24 10:05:05Z nces-hibino $
 */
...
vmcomtest(void)
{
	...
	*((unsigned int *)writedata) = vmcontest_cnt;
	ercd = WriteStateVariable(SVAR2, writedata);
	if (ercd != E_OK) {
		syslog(LOG_INFO, "WriteStateVariable(SVAR2) error! ercd = %d", ercd);
	}else {
		syslog(LOG_INFO, "WriteStateVariable(SVAR2) = %d", *((unsigned int *)writedata));
	}

	ercd = ReadStateVariable(SVAR1, readdata);
	if (ercd != E_OK) {
		syslog(LOG_INFO, "ReadStateVariable(SVAR1) error! ercd = %d", ercd);
	}
	else {
		syslog(LOG_INFO, "ReadStateVariable(SVAR1) = %d", *((unsigned int *)readdata));
	}

	ercd = ReadStateVariable(SVAR3, readdata);
	if (ercd != E_OK) {
		syslog(LOG_INFO, "ReadStateVariable(SVAR3) error! ercd = %d", ercd);
	}
	else {
		syslog(LOG_INFO, "ReadStateVariable(SVAR3) = %d", *((unsigned int *)readdata));
	}

	ercd = ReadStateVariable(SVAR4, readdata);
	if (ercd != E_OK) {
		syslog(LOG_INFO, "ReadStateVariable(SVAR4) error! ercd = %d", ercd);
	}
	else {
		syslog(LOG_INFO, "ReadStateVariable(SVAR4) = %d", *((unsigned int *)readdata));
	}

	ercd = WriteMessageQueue(MSGQ2, writedata, 4);
	if (ercd != E_OK) {
		syslog(LOG_INFO, "WriteMessageQueue(MSGQ2) error! ercd = %d", ercd);
	}
	else {
		syslog(LOG_INFO, "WriteMessageQueue(MSGQ2) = %d", *((unsigned int *)writedata));
	}

	ercd = ReadMessageQueue(MSGQ1, readdata);
	if (ercd < 0) {
		syslog(LOG_INFO, "ReadMessageQueue(MSGQ1) error! ercd = %d", ercd);
	}
	else {
		syslog(LOG_INFO, "ReadMessageQueue(MSGQ1) = %d", *((unsigned int *)readdata));
	}
...
GetCommand(void)
{
	...
	do {
		...
		cnt++;
		if (cnt == 100) {
			syslog(LOG_INFO, "ATK2 on VM2 : running! (%d)", run_cnt++);
			cnt = 0;
			vmcomtest();
		}
	} while (command == '\0');
